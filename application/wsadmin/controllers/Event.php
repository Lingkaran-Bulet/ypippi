...
class Event extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->__config = $this->config->item('ws_generic');
	...
	/** field insert mysql */
	public $path_pict = '/event/'; //name path picture
	public $path_images = 'event';
	...
	public $tags_input = 'ws_event_tags';
	public $user_create = 'ws_user_create';
	public $user_update = 'ws_user_update';
	public $status = 'ws_event_status_publish';
	public $metatitle_input = 'ws_event_metatitle';
	public $metakeyword_input = 'ws_event_metakeyword';
	public $metadesc_input = 'ws_event_metadesc';
	public $page = 20;
	public $count_image = 1;
	
	public $breadcumb = array('Content', 'Event'); //name module
	public $id_menu = 3; 
	
	public $table_image = 'ws_event_image'; 
	/** end field insert mysql */
	
	function index($null = null, $page = null){
		
		$offset = !empty($page) ? $this->page * ($page - 1) : 0;
		$this->event($offset, $page);
		
	}
	
	function event($offset, $page){
		...
		$forms['block_form'] = $this->load->view('module/event/block_list', $ws_result, true);
	...
	function add($data = null, $err = null){
		...
		isset($data[$this->event_date_input]) ?  $event_date_input = $data[$this->event_date_input]  : '' ;
		...
		$form['title'] = 'Post New Event';
		$form['form'] = array(
					'module' => $this->module
					,'datetimepicker_box' => datetimepicker_box(array('id' => $this->event_date_input
													,'id' => 'date-timepicker'
													,'class' => 'validate[required, custom[date]] col-xs-5 col-sm-10' 
													,'name'	=> $this->event_date_input
													,'value' => set_value($this->event_date_input, isset($event_date_input) ? $event_date_input : '')), $label = 'Date Event *')
		...
		$view = 'right/form';
		$forms['block_form'] = $this->load->view('module/event/block_form', $form, true);
	...
	function insert($post = null){
		...
		if(!empty($post) && $post == 'post'){
			...
			$this->form_validation->set_rules($this->event_date_input, 'Date Event', 'trim|required');
			...
			if ($this->form_validation->run() == TRUE){
				...
				$err[] = form_error($this->event_date_input, '<span class="error">', '</span>');
	...
	function edit($field_id=null, $data = null, $err = null){
		...
		$result = $this->querymodel->editRecord(array($this->id => $field_id)
												,$this->table
												,array('funct' => 'get_image', 'data' => array($this->id => $field_id), 'table' => $this->table_image));
		
		$event_date_input = isset($result[0][$this->event_date_input]) ? $result[0][$this->event_date_input] : '';
		...
		$form['form'] = array(
					...
					,'datetimepicker_box' => datetimepicker_box(array('id' => $this->event_date_input
													...
													,'class' => 'validate[required, custom[date]] col-xs-5 col-sm-10' 
													,'name'	=> $this->event_date_input
													,'value' => isset($event_date_input) ? $event_date_input : ''), $label = 'Date Event *')

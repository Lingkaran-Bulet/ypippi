...
class Querymodel extends  CI_Model {
	...
	function get_image($table, $data){
		...
		if (!empty($result))
		{
			...
			foreach ($result as $key => $val) {
				$index = ($val['ws_'.$this->module.'_image_index']-1);
				$temp[$index]['image_id'] = $val['ws_'.$this->module.'_image_id'];
				$temp[$index]['image_path'] = $val['ws_'.$this->module.'_image_path'];
			}			
		...
		foreach($_FILES as $name => $value){
			if(isset($value['size']) && $value['size'] > 0){
				...
				if(!empty($result_cek['error'])){
					...
					$param = array('table' => 'ws_'.$this->module.'_image', 
								'field_name' => $data_id, 
								'id' => $id,
	...
	function __insertImage ($param){
		...
		$data = array('ws_'.$this->module.'_image_date_create' => time()
					,'ws_'.$this->module.'_image_path' => $image
					,'ws_'.$this->module.'_image_index' => $number
					,$field_name => $id
				);
